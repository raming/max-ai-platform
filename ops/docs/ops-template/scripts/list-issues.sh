...
jq_filter='.[] | select(("'"$INCLUDE_BLOCKED"'"=="true") or ((.labels|map(.name)|index("blocked"))|not))'

# Compute priority weight from labels matching PRIORITY_PREFIX (P0=0,P1=1,...). Default weight=5 (low).
jq_sort='| .prio=((.labels|map(.name)|map(select(startswith("'"$PRIORITY_PREFIX"'")))|map(captures("P(?<p>[0-9]+)")|.captures[0].string|tonumber)|first) // 5)'

sorted=$(printf "%s" "$json" | jq -r "$jq_filter $jq_sort | {number,title,labels:[.labels[].name],updatedAt,prio} | @json" | sort -t$'\t' -k1,1)
# sort - we can use jq sort_by, but stream approach: use jq to emit lines and then use jq again to sort; better, use jq full sort:
sorted=$(printf "%s" "$json" | jq -r '[.[] | select(("'"$INCLUDE_BLOCKED"'"=="true") or ((.labels|map(.name)|index("blocked"))|not))
