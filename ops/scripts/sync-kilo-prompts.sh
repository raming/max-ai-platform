...
for role in "${ROLES_TO_GENERATE[@]}"; do
  ...
  # Use existing merge script with Kilo adaptations
   # Use existing merge script with Kilo adaptations
   # Get proper seat name from agents.yaml if available
   if [[ -f "$PROJECT_OPS_DIR/.agents/rules/agents.yaml" ]] && command -v yq >/dev/null 2>&1; then
     # Find the first available seat for this role
     SEAT=$(yq -r ".seats | keys[] | select(. | startswith(\"$role.\"))" "$PROJECT_OPS_DIR/.agents/rules/agents.yaml" 2>/dev/null | head -n 1) || SEAT="$role.default"
   else
     SEAT=\"$role.default\"
   fi

   ROLE=\"$role\" SEAT=\"$SEAT\" PROJECT_OPS_DIR=\"$PROJECT_OPS_DIR\" \
