...
#   LABELS           (optional)  comma-separated labels to add (e.g., status:needs-dev-fix,blocked)
#   SEAT_LABEL       (optional)  seat:<seat> label to apply (e.g., seat:dev.avery-kim)
#
# Example:
...
#   # QA handoff to Dev with blocked/fix labels:
#   LABELS="status:needs-dev-fix,blocked" SEAT_LABEL="seat:dev.avery-kim" ...
...
if [[ -n "${ISSUE:-}" ]] && command -v gh >/dev/null 2>&1; then
  ...
  # Base handoff labels
  base_labels="handoff/pending,blocked/waiting"
  all_labels="$base_labels"
  if [[ -n "$LABELS" ]]; then all_labels+=",$LABELS"; fi
  gh issue edit "$issue_ref" --add-label "$all_labels" >/dev/null 2>&1
  # Apply seat label if provided (for label-based routing)
  if [[ -n "$SEAT_LABEL" ]]; then
    gh issue edit "$issue_ref" --add-label "$SEAT_LABEL" >/dev/null 2>&1
  fi
  set -e
fi
