...
describe('Keycloak Token Claims Contract Validation', () => {
  ...
  describe('Invalid token claims', () => {
    it('should reject claims missing required iss field', () => {
      ...
      expect(result.errors![0].params.missingProperty).toBe('iss');
    });
    ...
    it('should reject claims missing required sub field', () => {
      ...
      const result = validateKeycloakTokenClaims(invalidClaims);
      expect(result.valid).toBe(false);
      expect(result.errors![0].params.missingProperty).toBe('sub');
    });
    ...
    it('should validate claims with non-URI iss since format validation is disabled', () => {
      const validClaims = {
        iss: 'not-a-uri',
        sub: 'user-123',
        aud: 'max-ai-platform',
